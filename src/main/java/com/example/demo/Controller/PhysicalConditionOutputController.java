...
@Controller
@RequestMapping("/app/PhysicalConditionOutput")
public class PhysicalConditionOutputController {
    ...
    String tag="";
    int id;
    Boolean MovingAverageFrag;
    int MovingAverageDenominator;
    ...
    @Autowired
    private DIBitAddmin BITs;
    @GetMapping
    public String GetOutput(@AuthenticationPrincipal UserDetails user, Model model) {
    	this.SetNumName = new ArrayList<>();
    	this.SetNumName.add("総合");
    	String userName = user.getUsername();
        List<NumericalName> NumNameList = namedao.findUser(userName);
        ...
        List<NumericalManagement> DataList = Managerdao.findByUserAndSort(userName);
        ...
        List<String> admins= menberdao.findAdmin(userName);
        List<MenberMessage> messages = new ArrayList<>();
        for(String admin:admins) {
        	List<MenberMessage> message=messagedao.FindMessage(admin, userName);
        	messages.addAll(message);
        }
        
        model.addAttribute("messages_output", messages);
        ...
        model.addAttribute("title", "Output page(" + userName + ")");
        ...
        for(NumericalName NumName:NumNameList) {
        	System.out.println(BITs.get_BIT_instance(NumName.getNumName(), userName));
    ...
    public String PostOutputNumName(Model model, @RequestParam("variable") String num_name) {
        ...
        return "redirect:/app/PhysicalConditionOutput";
    }
    @PostMapping("/year")
